...
function stringReverse(my_string: string, queries: number[][]) {  
    return queries.reduce((acc, value) => {
        const strArr = acc.split('');
        const reversArr = strArr.slice(value[0], value[1] + 1).reverse().join('');
        acc = strArr.slice(0, value[0]).join('') + reversArr + strArr.slice(value[1] + 1).join('');
        return acc;
    }, my_string);
}